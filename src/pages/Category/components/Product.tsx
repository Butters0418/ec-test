  ...
  return (
    <article>
      ...
      <div className="mt-0.5 xl:mt-2 flex items-center gap-1.5 xl:gap-2.5">
        ...
        {marketPrice && <p className="text-xl xl:text-2xl font-bold line-through text-black/40">${marketPrice.toLocaleString()}</p>}
        {!!discount && <p className="text-[10px] xl:text-xs font-medium text-[#FF3333] py-1 px-2 xl:px-3 xl:py-1.5 bg-[#FF3333]/10 rounded-4xl">-{discount}%</p>}
      </div>
    </article>
  );
}
